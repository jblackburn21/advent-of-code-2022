...
struct Monkey {
    item_worry_levels: VecDeque<u32>,
    op: fn(u32, u32) -> u32, // TODO: how to do partial fun?
    op_num: Option<u32>,
...
impl Monkey {
    fn new(starting_item_worry_levels: Vec<u32>,
           op: fn(u32, u32) -> u32,
           op_num: Option<u32>,
           test_divisible_by: u32,
           test_true_monkey: usize,
           test_false_monkey: usize,
           init_items_held: u32) -> Monkey {
    ...
    fn push_back(&mut self, item_worry_level: u32) {
...
pub fn main() {
    ...
    let mut monkeys: Vec<Monkey> = include_str!("../input.txt")
        ...
        .map(|chunk| {
            ...
            let starting_items: Vec<u32> = chunk.lines().nth(1).unwrap()
                                            .split_once(":").unwrap().1.trim()
                                            .split(",")
                                            .map(|si| si.trim().parse::<u32>().unwrap())
                                            .collect();
            ...
            let op_num = op_line[2].parse::<u32>().ok();
            ...
            let init_items_held = starting_items.len() as u32;
    ...
    for round in 0..20 {
        ...
        for idx in 0..monkeys_count {
            ...
            while let Some(item_worry_level) = m.pop_front() {
                ...
                let new_worry_level = (m.op)(item_worry_level, n);

                let div_by_3 = ((new_worry_level / 3) as f32).floor() as u32;

                let is_divisible_by = div_by_3.rem_euclid(m.test_divisible_by) == 0;

                let to_monkey = if is_divisible_by { m.test_true_monkey } else { m.test_false_monkey };

                moves.push((to_monkey,div_by_3));
    ...
    let mut levels: Vec<u32> = monkeys.iter()
                        .map(|m|{
                            // subtract the current levels since they haven't been inspected
                            m.items_held - (m.item_worry_levels.len() as u32)
                        })
                        .collect();
...
fn add_op(old: u32, num: u32) -> u32 {
    old + num
}

fn mult_op(old: u32, num: u32) -> u32 {
    old * num
}
