...
pub fn main() {
    ...
    let fs_disk_space: u32 = 70000000;
    let required_disk_space: u32 = 30000000;
    let root_size = dir_sizes_clone2.get(CD_TOP_LEVEL).unwrap();
    let available_space = fs_disk_space - root_size;

    let delete_dir_size =  dir_sizes_clone2.values()
        .filter(|&s| *s + available_space > required_disk_space)
        .min().unwrap();

    println!("Delete dir size: {delete_dir_size}");
}
